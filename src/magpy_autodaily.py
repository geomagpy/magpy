...
#Some definitions
endtime=datetime.strptime('2012-9-11T23:59:00',"%Y-%m-%dT%H:%M:%S") # datetime.replace by utcnow()
...
try:
    ...
    stDIDD = stDIDD.delta_f(offset=deltaF) # use last years peardiff
...
variopath = os.path.join(basepath,'LEMI-WIK','*.min')
scalarpath = os.path.join(basepath,'DIDD-WIK','*')
...
try:
    ...
    headers['DigitalSamplingInterval'] = '0.1 Hz'
