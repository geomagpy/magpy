...
class PubSubClient(WampClientProtocol):
    ...
    def subscribeInst(self, db, cursor, client):
        ...
        sql = "SELECT SensorID, SensorDescription, SensorName, SensorKeys FROM SENSORS WHERE SensorName = 'POS1'"
        try:
    ...
    def onEvent(self, topicUri, event):
        ...
        try:
            ...
            if eol == '':
                ...
                sql = "INSERT INTO %s(time, f, flag, typ) VALUES ('%s', %f, '0000000000000000-', 'xyz')" % (sensorid, self.line[0], self.line[1])
                self.line = []
