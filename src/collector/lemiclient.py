...
class PubSubClient(WampClientProtocol):
    ...
    def subscribeInst(self, db, cursor, client):
        self.prefix("lemi", "http://example.com/" + client +"/lemi#")
        sql = "SELECT SensorID, SensorDescription, SensorDataLogger, SensorKeys FROM SENSORS WHERE SensorDataLogger = 'Lemi'"
        try:
    ...
    def onEvent(self, topicUri, event):
        ...
        try:
            ...
            if eventdict['id'] == 99:
                eol = eventdict['value']
            if eol == '':
                if eventdict['id'] in [1,2,3,5,6,8]: # replace by some eol parameter
