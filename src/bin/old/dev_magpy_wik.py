...
st = read(path_or_url=os.path.join(mainpath,'DIDD-WIK','*'),starttime= str(year)+'-07-20', endtime=str(year)+'-07-31')
st.plot(['x','y'],plottype='continuous')
st.write(os.path.join(mainpath,'DIDD-WIK','data'),filenamebegins='DIDD_',format_type='PYCDF')
st.header.clear()
st = read(path_or_url=os.path.join(mainpath,'DIDD-WIK','data','*'),starttime= str(year)+'-07-20', endtime=str(year)+'-07-31')
st.plot(['x','y','t1'],plottype='continuous')
print datetime.utcnow()
...
# 1. Get data
st2 = read(path_or_url=os.path.join(mainpath,'LEMI-WIK','*'),starttime= str(year)+'-01-01', endtime=str(year+1)+'-01-01')
# 2. Merge auxilliary data
...
# 1. Get data
stPMAG = read(path_or_url=os.path.join(mainpath,'PMAG-WIK','*'),starttime= str(year)+'-01-01', endtime=str(year+1)+'-01-01')
