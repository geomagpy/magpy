...
# write the data
abslemi.pmwrite(writeresultpath,coverage='all',mode='replace',filenamebegins='WIK-D-LEMI_1_BE_01')
# make plot covering one year back from now
abslemi = pmRead(path_or_url=os.path.join(writeresultpath,'WIK-D-LEMI_1_BE_01.txt'))
abslemi = abslemi.routlier(timerange=timedelta(days=60),keys=['dx','dy','dz'],threshold=1.5)
abslemi.pmwrite(writeresultpath,coverage='all',filenamebegins='WIK-D-LEMI_1_BE_01')
...
absdidd.pmwrite(writeresultpath,coverage='all',mode='replace',filenamebegins='WIK-D-DIDD_3121331_S_01')
absdidd = pmRead(path_or_url=os.path.join(writeresultpath,'WIK-D-DIDD_3121331_S_01.txt'))
absdidd = absdidd.routlier(timerange=timedelta(days=60),keys=['dx','dy','dz'],threshold=1.5)
absdidd.pmwrite(writeresultpath,coverage='all',filenamebegins='WIK-D-DIDD_3121331_S_01')
