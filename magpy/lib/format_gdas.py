    ...
    if getfile:
        ...
        for line in fh:
            if line.isspace():
                ...
                elem = line.split()
                try:
                    array[0].append(datetime.strptime(elem[0],"%d-%m-%Y-%H:%M:%S"))
                except:
                    try:
                        array[0].append(datetime.strptime(elem[0],"%Y-%m-%dT%H:%M:%S"))
                    except:
...
def readGDASB1(filename, headonly=False, **kwargs):
    ...
    if getfile:
        ...
        while line != "":
            ...
            if not data[4] == 99999:
                array[1].append(float(data[4])/10.0)
            if not data[5] == 99999:
                array[2].append(float(data[5])/10.0)
            if not data[6] == 99999:
                array[3].append(float(data[6])/10.0)
            if not data[7] == 99999:
                array[5].append(float(data[7])/10.0)
            if not data[8] == 99999:
                array[4].append(float(data[8])/10.0)
            line = fh.read(25)
