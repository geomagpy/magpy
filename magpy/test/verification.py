...
class TestStream(unittest.TestCase):
    ...
    def test_resample(self):
        #print (len(teststream), teststream.ndarray)
        ...
        # The 2.0 method will have some variations because of the fourier method
        self.assertAlmostEqual(np.abs(diff.mean('x',meanfunction="median", percentage=50)), 0.0, 5)
        self.assertAlmostEqual(np.abs(diff.mean('y',meanfunction="median", percentage=50)), 0.0, 5)
        self.assertAlmostEqual(np.abs(diff.mean('z',meanfunction="median", percentage=50)), 0.0, 5)
        self.assertEqual(len(teststream), len(resampleddata) * 2)

    def test_rotation(self):
        cutstream = teststream.cut(50, kind=0, order=1)
        pmeanx = cutstream.mean('x',percentage=50)
        pmeany = cutstream.mean('y',percentage=50)
        pmeanz = cutstream.mean('z',percentage=50)
        print (pmeanx, pmeanz)
        cpstream = cutstream.copy()
        # testing horizontal rotation
        protstream = cutstream.rotation(alpha=45.)
        smeanx = protstream.mean('x', percentage=50)
        smeany = protstream.mean('y', percentage=50)
        ...
        tmeanx = srotstream.mean('x',percentage=50)
        tmeany = srotstream.mean('y',percentage=50)
        tmeanz = srotstream.mean('z',percentage=50)
        ...
        meanx = rotstream.mean('x',percentage=50)
        meany = rotstream.mean('y',percentage=50)
        meanz = rotstream.mean('z',percentage=50)
        ...
        omeanx = orgstream.mean('x',percentage=50)
        omeany = orgstream.mean('y',percentage=50)
        omeanz = orgstream.mean('z',percentage=50)
