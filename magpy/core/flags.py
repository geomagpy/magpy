...
class flags(object):
    ...
    def l2d(self,flaglist=None):
        ...
        mydic = {}
        # if flagversion is contained in list, then remove it
        if not len(mylist) > 0:
            return {}
        if len(mylist[0]) > 7:
                    ...
                    while idxtmp < len(complist):
                        ...
                        for idx,line in enumerate(complist):
                            ...
                            if tdiff > sr-(0.05*sr):
                                ...
                                for step in np.arange(steps):
                                    ...
                                    extendedcomplist.append([val0,val0,line[2],line[3],line[4],line[5],line[6],line[7]])
                                extendedcomplist.append([lastt,lastt,line[2],line[3],line[4],line[5],line[6],line[7]])
                            else:
                            ...
                            idxtmp = idx+1
                        if debug:
                            print ("    -> extending flags to individual time stamps: {}".format(len(extendedcomplist)))
                        if overlap:
                            if debug:
                                print ("    -> removing overlaps")
                            # Now sort the extendedlist according to modification date
                            extendedcomplist.sort(key=lambda x: x[-2], reverse=True)
                            ...
                            if debug:
                                print ("    -> after overlap removal - extended time stamps: {}".format(len(extendedcomplist)))
                        ...
                        for idx,line in enumerate(extendedcomplist):
                            ...
                            if startt and endt:
                                # add new line
                                if not remove:
                                    new2list.append([startt,endt,line[2],line[3],line[4],line[5],line[6],line[7]])
                                    newflaglist.append([startt,endt,line[2],line[3],line[4],line[5],line[6],line[7]])
                                else:
